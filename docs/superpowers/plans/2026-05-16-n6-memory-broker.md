# N6 Memory Broker Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the N6 Memory Broker as an MCP Server — a centralized, always-on memory daemon that serves all N1-N9 agents with persistent, searchable, access-controlled long-term memory.

**Architecture:** N6 is a Single-Writer MCP Server backed by a dual-layer store: Markdown files as Source of Truth + SQLite/FTS5 as derived index. Gemini Embedding 2 provides semantic search with FTS5 fallback. ACL enforces namespace isolation — managers (N1/N6/N7/N9) have global access, general agents read/write only their own namespace.

**Tech Stack:** Python 3.13+, SQLite 3.49+ (WAL + FTS5), MCP SDK (`mcp[cli]`), Google Generative AI (Embedding), pathlib, pytest

**Decisions Reference:** KI `n6-memory-architecture-research/artifacts/接續指南.md` §5.1
...
## Task 1: Project Skeleton + MCP Server Bootstrap

**Files:**
- Create: `src/n6_memory_broker/__init__.py`
- Create: `src/n6_memory_broker/config.py`
- Create: `src/n6_memory_broker/server.py`
...
## Task 2: SQLite Schema + FTS5 Index + Pydantic Models

**Files:**
- Create: `src/n6_memory_broker/schema.py`
- Create: `src/n6_memory_broker/models.py`
...
## Task 3: ACL (Access Control Layer)
...
## Task 5: MCP Write Tools (memory_submit + memory_update)

**Files:**
- Create: `src/n6_memory_broker/tools/__init__.py`
...
## Task 8: Embedding Layer (Gemini + FTS5 Fallback)
...
## Task 9: γ-Decay Engine (Tier Promotion + Archival)
...
## Task 10: Legacy Migration Script
...
## Task 11: MCP Tool Registration + Server Integration
...
- `CallerIdentity` used consistently across ACL, write_tools, read_tools, admin_tools
- `MemoryEntry` used consistently in store + write_tools + legacy_migrator
- `insert_memory()` / `get_memory()` / `search_fts()` signatures stable across all consumers

### 4. Deferred Items (Not in This Plan)

| Item | Reason | Suggested Follow-up |
|------|--------|-------------------|
| Markdown ↔ DB sync (`markdown_sync.py`) | Depends on Markdown directory structure decisions | Separate plan after Phase 1 |
| 2h Inspector daemon (`inspector.py`) | Requires scheduler integration (cron/Temporalio) | Separate plan |
| Web UI channel (Q7 Phase 2) | Future scope | Separate plan |
| Line channel (Q7 Phase 3) | Future scope | Separate plan |
| Vector index (sqlite-vec or FAISS) | Q3 optional optimization | Performance plan |

---

**Plan complete and saved to `docs/superpowers/plans/2026-05-16-n6-memory-broker.md`. Two execution options:**
