"""fix_antigravity2_noninteractive.py — 非互動版修復腳本
複製自 fix_antigravity2.py，移除 input() 等待
"""
import sqlite3
import shutil
import os
...
print("=" * 60)
print("  Antigravity 2 對話歷史修復腳本（非互動版）")
print("=" * 60)
...
if not os.path.exists(old_db):
    print(f"  ❌ 找不到 state.vscdb: {old_db}")
    print("  ⚠️  Antigravity 2 可能尚未安裝或目錄不同")
    sys.exit(1)
...
if row and row[0]:
    ...
    if os.path.exists(ide_db):
        ...
        if ide_row:
            ...
            print("  ❌ IDE 中也沒有 trajectorySummaries")
        ide_conn.close()
    else:
        print(f"  ❌ 找不到 IDE state.vscdb: {ide_db}")
