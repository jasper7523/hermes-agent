...
function showAndLog(msg: string, isError = false) {
  log(msg);
  if (isError) {
  ...
  try {
    ...
    const messages = [
      vscode.LanguageModelChatMessage.User(
        "Reply with exactly: HELLO_FROM_LM_PROBE_SUCCESS"
      ),
    ];
...
async function fullReport() {
  ...
  if (!vscode.lm) {
    ...
    for (const key of keys) {
      const val = (vscode.lm as any)[key];
      log(`  .${key}: ${typeof val}`);
    }
  }
  ...
  log(
    `vscode.chat: ${typeof (vscode as any).chat !== "undefined" ? "EXISTS" : "undefined"}`
  );
  log(
    `vscode.ai: ${typeof (vscode as any).ai !== "undefined" ? "EXISTS" : "undefined"}`
  );

  // Check extensions
  log("\n--- Installed AI Extensions ---");
  const aiExtensions = vscode.extensions.all.filter(
    (ext) =>
      ext.id.toLowerCase().includes("gemini") ||
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("lmProbe.checkModels", checkModels),
    vscode.commands.registerCommand("lmProbe.testGenerate", testGenerate),
    vscode.commands.registerCommand("lmProbe.fullReport", fullReport),
    outputChannel
  );
}
